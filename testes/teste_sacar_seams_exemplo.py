...
def test_sacar_grava_data_hora_exata(banco_temp, usuario_registrado):
    conta_id = _conta_id_de(usuario_registrado)
    servico.depositar(conta_id=conta_id, valor=200.0)

    data_fixa = datetime(2026, 1, 1, 12, 0, 0)
    servico.sacar(conta_id=conta_id, valor=50.0, agora=data_fixa)
...
def test_sacar_usa_now_quando_agora_nao_informado(banco_temp, usuario_registrado):
    conta_id = _conta_id_de(usuario_registrado)
    servico.depositar(conta_id=conta_id, valor=200.0)

    antes = datetime.now()
    servico.sacar(conta_id=conta_id, valor=30.0)
    depois = datetime.now()
